<div align="center">

# 🛠️ Useful Snippets

**A curated collection of handy code snippets, shell functions, and developer tips**

[![GitHub stars](https://img.shields.io/github/stars/leminhnguyen/useful_snippets?style=flat-square)](https://github.com/leminhnguyen/useful_snippets/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/leminhnguyen/useful_snippets?style=flat-square)](https://github.com/leminhnguyen/useful_snippets/network)
[![Contributions welcome](https://img.shields.io/badge/contributions-welcome-brightgreen?style=flat-square)](https://github.com/leminhnguyen/useful_snippets/pulls)

</div>

---

## 📚 Table of Contents

- [🐚 Bash Snippets](#-bash-snippets)
- [🐍 Python Snippets](#-python-snippets)
- [🌟 Interesting Topics](#-interesting-topics)
- [📝 Citation](#-citation)
- [🤝 Contributing](#-contributing)

---

## 🐚 Bash Snippets

Useful shell functions and one-liners for everyday terminal work.

| # | Snippet | Description |
|---|---------|-------------|
| 1 | `fcount` | Count the number of files in a folder |
| 2 | Fix CUDA | Fix CUDA force-overwrite/broken install issues |
| 3 | `nbl-count` | Count non-blank lines in a file |
| 4 | Docker no-sudo | Run Docker without `sudo` |
| 5 | Fix 2nd screen | Recover second monitor after reinstalling Ubuntu |
| 6 | Fix DISPLAY | Fix `MIT-MAGIC-COOKIE-1` / `DISPLAY` errors |
| 7 | `sizeof` | Get the size of a folder |
| 8 | Rsync | Sync files from local to remote server |
| 9 | Kill process | Find and kill specific processes by name |
| 10 | `downloading.sh` | Download multiple files into a folder via CLI |
| 11 | Recover tmux | Recover a lost tmux session |

➡️ [View all Bash snippets →](bash/README.md)

---

## 🐍 Python Snippets

Reusable Python patterns and utilities to speed up development.

| # | Snippet | Description |
|---|---------|-------------|
| 1 | `p_tqdm` multiprocessing | Parallel processing with a live progress bar |
| 2 | Fast audio duration | Read audio length without decoding the full file |
| 3 | Time decorator | Measure execution time of any function |
| 4 | Fix `gdown` permission | Download Google Drive files from Colab/Kaggle |

➡️ [View all Python snippets →](python/README.md)

---

## 🌟 Interesting Topics

Step-by-step guides on useful development workflows.

| Topic | Description |
|-------|-------------|
| [Use Colab/Kaggle in VS Code](interesting_topics/colabcode.md) | Connect cloud notebooks to your local VS Code via `ngrok` |
| [Linux server with Windows Terminal](interesting_topics/window_terminal.md) | Easy SSH profile setup using Windows Terminal |

---

## 📝 Citation

All code in this repository is free to use. A reference back to the repo is appreciated:

```bash
# Source: https://github.com/leminhnguyen/useful_snippets
```

Example in `~/.bashrc`:

```bash
# the code got from https://github.com/leminhnguyen/useful_snippets
sizeof() {
    # example: sizeof BOOK  -->  895M    BOOK/
    du -h --max-depth=0 "$1"
}
```

---

## 🤝 Contributing

Contributions are welcome! Feel free to:

- Open a **Pull Request** with new snippets or fixes
- Open an **Issue** to suggest improvements
- ⭐ Star the repo if you find it useful!
